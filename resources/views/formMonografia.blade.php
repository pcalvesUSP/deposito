...
@section('content')
  ...
  <form id="formMonografia" method="post" action="@if ($edicao) {{ route("alunos.corrigir",['id'=>$dadosMonografia->id]) }} @else {{ route("salvarMonografia") }} @endif" enctype="multipart/form-data">
    <fieldset id="fieldsMonografia" class="grupo">
      ...
      @if ($publicar)
        <div class="campo">
          <label for="publicar">Publicar Trabalho no BDTA?</label>
      ...
      @if (isset($dadosMonografia->template_apres) && !empty($dadosMonografia->template_apres)) 
        <b><a href="upload/{{ $dadosMonografia->template_apres }}">Baixar o trabalho</a></b>
      @endif
      @if (!$readonly && ($userLogado == "Graduacao" || $userLogado == "Aluno"))
      <div class="campo">
        <label for="template_apres">@if ($userLogado == "Graduacao") Atualizar o arquivo  @else Arquivo do TCC: @endif </label><input type="file" name="template_apres" id="template_apres">
